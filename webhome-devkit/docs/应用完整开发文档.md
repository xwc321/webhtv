...
脚本读取 `third_party/mpv-native-lock.json`，自动应用 MPV 光盘控制补丁；所有下载和中间文件放在被 Git 忽略的 `build/mpv-native/`。当前 lock 与两套已提交 assets 一致，可复现正式组合。Gradle 和 GitHub Actions 始终复用仓库已提交的 `.so`，不会现场重编 MPV；Android Release Action 会先运行 `scripts/verify_mpv_native_assets.sh --require-elf`，检查两套 assets 的文件集合、ABI、版本标记、`SONAME` 和 `DT_NEEDED`。完整排查记录见本地 `plans/MPV原生依赖升级与Android崩溃排查记录.md`。
